...
public class OrderLineAssert extends AbstractObjectAssert<OrderLineAssert, OrderLine> {
  ...
  /**
   * Verifies that the actual OrderLine's stockItem is equal to the given one.
   * @param stockItem the given stockItem to compare the actual OrderLine's stockItem to.
   * @return this assertion object.
   * @throws AssertionError - if the actual OrderLine's stockItem is not equal to the given one.
   */
  public OrderLineAssert hasStockItem(com.kagzz.jmix.rys.product.entity.StockItem stockItem) {
    // check that actual OrderLine we want to make assertions on is not null.
    isNotNull();

    // overrides the default error message with a more explicit one
    String assertjErrorMessage = "\nExpecting stockItem of:\n  <%s>\nto be:\n  <%s>\nbut was:\n  <%s>";

    // null safe check
    com.kagzz.jmix.rys.product.entity.StockItem actualStockItem = actual.getStockItem();
    if (!Objects.deepEquals(actualStockItem, stockItem)) {
      failWithMessage(assertjErrorMessage, actual, stockItem, actualStockItem);
    }
